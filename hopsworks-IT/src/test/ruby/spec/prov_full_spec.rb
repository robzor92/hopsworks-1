...
describe "On #{ENV['OS']}" do
  ...
  describe 'test suite - 1 project' do
    ...
    describe "ml asset (experiment) xattr tests" do
      it "add xattr" do
        ...
        test_xattr_field = fixed_json["test_xattr"]["value"]
      ...
      it "not json xattr" do
        ...
        expect(prov_xattr.length).to eq 1
        fixed_json = fix_json(prov_xattr[0]["value"])
        test_xattr_field = fixed_json["test_xattr"]["raw"]
  ...
  describe 'test suite - one project - reuse - no state change tests' do
      before :all do
        pp "create project: #{@project1_name}"
        @project1 = create_project_by_name(@project1_name)
      end
...
#       end
  ...
  describe 'test suite - one project - with state' do
    before :all do
      pp "create project: #{@project3_name}"
      @project3 = create_project_by_name(@project3_name)
    end
