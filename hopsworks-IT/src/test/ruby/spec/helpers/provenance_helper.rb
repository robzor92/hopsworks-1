...
module ProvenanceHelper
  ...
  def prov_check_experiment1(experiments, experiment_id) 
    experiment = experiments.select { |e| e["mlId"] == experiment_id }
    expect(experiment.length).to eq 1
  end

  def prov_check_experiment2(experiments, experiment_id, xattrs) 
    experiment = experiments.select { |e| e["mlId"] == experiment_id }
    expect(experiment.length).to eq 1
    #pp experiment
  ...
  def prov_check_model(models, model_id, xattrs) 
  ...
  def prov_check_td1(tds, td_name, td_version) 
    td = tds.select {|t| t["mlId"] == prov_td_id(td_name, td_version) }
    expect(td.length).to eq 1
    pp model
  end 
