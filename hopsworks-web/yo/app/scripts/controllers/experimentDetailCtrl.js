                    ...
                    if (results) {
                        if (results[0]['parameters']) {
                            ...
                            for (var z = 0; z < hyperparameters.length; z++) {
                                var key = Object.keys(hyperparameters[z])[0]
                                self.hp_headers.push(key)
                            }
                        }

                        var metrics = results[0]['metrics'];
                        for (var x = 0; x < metrics.length; x++) {
                            var key = Object.keys(metrics[x])[0]
                            self.metric_headers.push(key);
                        }
                        ...
                        for (var i = 0; i < results.length; i++) {
                            ...
                            if (results[i]['parameters']) {
                                for (var y = 0; y < results[i]['parameters'].length; y++) {
                                    var key = Object.keys(results[i]['parameters'][y])[0];
                                    tmp.push({
                                        'data': results[i]['parameters'][y][key],
                                        'file': false
                                    })
                                }
                            }
                            if (results[i]['metrics']) {
                                for (var y = 0; y < results[i]['metrics'].length; y++) {
                                    var key = Object.keys(results[i]['metrics'][y])[0];
                                    var is_file = String(results[i]['metrics'][y][key]).indexOf('/') > -1;
                                    tmp.push({
                                        'data': results[i]['metrics'][y][key],
                                        'file': is_file
                                    });
                                }
                            }
